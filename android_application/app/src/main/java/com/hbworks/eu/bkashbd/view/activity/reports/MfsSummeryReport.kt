...
import com.hbworks.eu.bkashbd.databinding.ActivityMfsSummeryReportBinding
...
class MfsSummeryReport : BaseActivity() {
    ...
    private var mfsSummeryReport:MutableList<ArrayList<String>> =  mutableListOf()

    private val binding: ActivityMfsSummeryReportBinding by lazy {
        ActivityMfsSummeryReportBinding.inflate(layoutInflater)
    }
    ...
    override fun viewRelatedTask() {
        ...
        FilterCriteria["date_from"] = "From: ${(DateTime.now()).format(DateFormat("yyyy-MM-dd"))}||${(DateTime.now()).format(DateFormat("yyyy-MM-dd"))}"
        FilterCriteria["date_to"] = "To: ${DateTime.now().format(DateFormat("yyyy-MM-dd"))}||${DateTime.now().format(DateFormat("yyyy-MM-dd"))}"

        binding.FromDate.setText((DateTime.now()).format(DateFormat("yyyy-MM-dd")))
        ...
        viewModel.forceLogOut.observe(this, androidx.lifecycle.Observer {
            ...
            startActivity(Intent(this@MfsSummeryReport, LoginActivity::class.java))
            finish()
        })
        ...
        viewModel.mfsSummeryReportResponse.observe(this, androidx.lifecycle.Observer {
            mfsSummeryReport.clear()
            mfsSummeryReport.addAll(it!!.data!!.toMutableList())
    ...
    fun reload()
    {
        ...
        searchParams["type"] = userDetails.userType!!
        viewModel.getMfsWiseRechargeSummeryReport(searchParams)
    }

    class TheRecyclerAdapter(itemView: ViewDataBinding, context: Context) : BaseViewHolder(itemView.root) {

        var binding = itemView as RvTable3cT1Binding
