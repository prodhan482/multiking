...
import com.hbworks.eu.bkashbd.databinding.ActivityAddBalanceReportBinding
...
class AddBalanceReport : BaseActivity() {

    private val binding: ActivityAddBalanceReportBinding by lazy {
        ActivityAddBalanceReportBinding.inflate(layoutInflater)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        this.viewModel = ViewModelProviders.of(this).get(ReportViewModel::class.java)
    }

    override fun viewRelatedTask() {

        FilterCriteria["date_from"] = "From: ${(DateTime.now() - 30.days).format(DateFormat("yyyy-MM-dd"))}||${(DateTime.now() - 30.days).format(
            DateFormat("yyyy-MM-dd")
        )}"
        FilterCriteria["date_to"] = "To: ${DateTime.now().format(DateFormat("yyyy-MM-dd"))}||${
            DateTime.now().format(
                DateFormat("yyyy-MM-dd")
            )}"
        ...
        viewModel.forceLogOut.observe(this, androidx.lifecycle.Observer {
            ...
            startActivity(Intent(this@AddBalanceReport, LoginActivity::class.java))
            finish()
        })
    ...
    fun reload()
    {
        ...
        searchParams["trans_type"] = "add_balance"
    ...
    class TheRecyclerAdapter(itemView: ViewDataBinding, context: Context, val userDetails:UserProfileData) : BaseViewHolder(itemView.root) {
        ...
        override fun <T> onBind(position: Int, model: T, mCallback: IAdapterListener) {
            ...
            binding.col3.setText("${if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                Html.fromHtml("${model[8]}", Html.FROM_HTML_MODE_COMPACT)
            } else {
                Html.fromHtml("${model[8]}")
            }}".replace("BDT", "৳"))

            binding.col4.setText("${if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                Html.fromHtml("${model[9]}", Html.FROM_HTML_MODE_COMPACT)
            } else {
                Html.fromHtml("${model[9]}")
            }}".replace("BDT", "৳"))
        }
    }
}
