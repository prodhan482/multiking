...
class ResellersList : BaseActivity() {
    ...
    private val binding: ActivityResellersListBinding by lazy {
        ActivityResellersListBinding.inflate(layoutInflater)
    }
    ...
    override fun viewRelatedTask() {
        ...
        binding.SearchBoxContent.doOnTextChanged { text, start, before, count ->
            binding.searchBtt.isEnabled = (count > 0)
        }

        binding.addNewBtt.setOnClickListener {
            startForResult(Intent(this@ResellersList, AddNewReseller::class.java)){
                reload()
            }.onFailed { result ->
        ...
        viewModel.forceLogOut.observe(this, androidx.lifecycle.Observer {
            ...
            startActivity(Intent(this@ResellersList, LoginActivity::class.java))
            finish()
        })
        ...
        binding.theList.adapter = BaseRecyclerAdapter(window.context, object : IAdapterListener {
            override fun <T> clickListener(position: Int, model: T, view: View) {
                ...
                when(position)
                {
                        ...
                        setBottomDialogListener(object :
                            ...
                            override fun onYesPressed(info: HashMap<String, String>) {
                                ...
                                if(!info.containsKey("note")) info["note"] = ""
                                    MaterialDialog(this@ResellersList).show {
                    ...
                    1->{
                        // Change Status
                        InputSheet().show(this@ResellersList) {
   ...
   class TheRecyclerAdapter(itemView: ViewDataBinding, context: Context, val permissions:ArrayList<String>) :
        ...
        override fun <T> onBind(position: Int, model: T, mCallback: IAdapterListener) {
            ...
            if(model[4].toFloat() < 0) binding.euroBalance.setTextColor(mContext.resources.getColor(R.color.md_green_c1))
            if(model[4].toFloat() >= 0) binding.euroBalance.setTextColor(mContext.resources.getColor(R.color.md_red_A700))
