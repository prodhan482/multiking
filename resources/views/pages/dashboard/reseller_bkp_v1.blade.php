...
<div class="app-content content " id="app">
    ...
    <div class="content-wrapper">
        ...
        <div class="content-body">
                ...
                <div class="row match-height">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <div class="row match-height">
                                    <div :class="[(isMobile()?'col-4':'col')]" style="text-align: center"
                                         v-for="option in mfs_list">
                                        <a href="javascript:void(0)" v-on:click="selectTheOption(option.mfs_id)">
                                            <img :src="'/'+option.image_path" class="img-fluid" :alt="option.mfs_name">
                                        </a>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="col-lg-6 col-12">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-control select-mfs" data-fouc v-model="request.selected_mfs">
                                            ...
                                            <option v-for="option in mfs_list" :value="option.mfs_id" v-html="option.mfs_name"></option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group row" v-if="mfs_package_list[request.selected_mfs] && mfs_package_list[request.selected_mfs].length > 0">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-control mfs-package" v-model="request.selected_mfs_package">
                                            ...
                                            <option v-for="option in mfs_package_list[request.selected_mfs]" :value="option.row_id" v-html="option.package_name"></option>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group row">
                        ...
                        <span class="input-group-text">BDT</span>
                                ...
                                <div class="form-group row" v-if="(parseFloat(request.charge) > 0)">
                                    ...
                                    <div class="col-sm-9" v-html='("BDT "+toNum(request.send_money * request.charge)+"/=")'>
                                    </div>
                                </div>
                                <div class="form-group row" v-if="(parseFloat(request.commission) > 0)">
                                    <label class="col-form-label col-sm-3">Commission</label>
                                    <div class="col-sm-9" v-html='("BDT "+toNum(request.send_money * request.commission)+"/=")'>
                                    </div>
                                </div>
            ...
            <div class="modal" tabindex="-1" role="dialog" id="doTransfer">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Do a Transfer</h5>
                        ...
                        <div class="modal-body">
                            <div class="form-group row">
                                <label class="col-form-label col-sm-3">MFS</label>
                                <div class="col-sm-9">
                                    <select class="form-control select-mfs2" data-fouc v-model="request.selected_mfs">
                                        <option value="">Select a Option</option>
                                        <option v-for="option in mfs_list" :value="option.mfs_id" v-html="option.mfs_name"></option>
                                    </select>
                                </div>
                            ...
                            <div class="form-group row" v-if="mfs_package_list[request.selected_mfs] && mfs_package_list[request.selected_mfs].length > 0">
                                <label class="col-form-label col-sm-3">Package</label>
                                <div class="col-sm-9">
                                    <select class="form-control mfs-package2" v-model="request.selected_mfs_package">
                                        <option value="">Select a Package</option>
                                        <option v-for="option in mfs_package_list[request.selected_mfs]" :value="option.row_id" v-html="option.package_name"></option>
                                    </select>
                                </div>
                  ...
                  <span class="input-group-text">BDT</span>
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-primary">Do Recharge</button>
    ...
    $(function(){
        var app = new Vue({
            ...
            mounted() {
                ...
                $('.mfs-package, .mfs-package2').select2({ width: '100%' });
                $('.select-mfs, .select-mfs2').select2({ width: '100%' });
                $('.select-mfs, .select-mfs2').on('select2:select', function (e) {
                    if($('.mfs-package, .mfs-package2').hasClass("select2-hidden-accessible")){ $('.mfs-package, .mfs-package2').select2('destroy') };
                    theInstance.request.selected_mfs = $('.select-mfs, .select-mfs2').select2('data')[0].id
                    //console.log(theInstance.mfs_package_list[theInstance.request.selected_mfs])
                    setTimeout(function(){
                        $('.mfs-package, .mfs-package2').select2({ width: '100%' });
                        $('.mfs-package, .mfs-package2').on('select2:select', function (e) {

                            theInstance.request.selected_mfs_package = $('.mfs-package, .mfs-package2').select2('data')[0].id

                            if(theInstance.mfs_package_list_id[$('.mfs-package, .mfs-package2').select2('data')[0].id])
                            {
                                var ll = $('.mfs-package, .mfs-package2').select2('data')[0].id
            ...
            methods: {
                ...
                selectTheOption(mfs_id)
                {
                    $('#doTransfer').modal('show')
                    $('.select-mfs, .select-mfs2').val(mfs_id).trigger('change');
                },
                sendARequest()
                {
