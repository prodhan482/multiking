...
class ProgressHud extends StatefulWidget {
  ...
  static Future showSuccessAndDismiss({String? text}) async {
    return _globalHud?.showSuccessAndDismiss(text: text == null ? "" : text);
  }

  /// show error icon with text and dismiss automatic
  static Future showErrorAndDismiss({String? text}) async {
    return _globalHud?.showErrorAndDismiss(text: text == null ? "" : text);
  }
...
class ProgressHudState extends State<ProgressHud> {
  ...
  Future showSuccessAndDismiss({String? text}) async {
    await this
        .showAndDismiss(ProgressHudType.success, text == null ? "" : text);
  }

  /// show error icon with text and dismiss automatic
  Future showErrorAndDismiss({String? text}) async {
    await this.showAndDismiss(ProgressHudType.error, text == null ? "" : text);
  }
    ...
    switch (_progressType) {
      case ProgressHudType.loading:
        var sizeBox = SizedBox(
          ...
          height: kIconSize,
          child: Image.asset(
            AppConstants.imagePath + "loader.gif",
            // height: 60.s,
            // width: 60.s,
          ),
          //n child: CupertinoTheme(data: CupertinoTheme.of(context).copyWith(brightness: Brightness.dark), child: CupertinoActivityIndicator(animating: true, radius: 16),),
        );
    ...
    return Stack(
      children: <Widget>[
        ...
        Center(
          child: Container(
            margin:
                EdgeInsets.fromLTRB(10, 10, 10, 10 /* - widget.offsetY * 2*/),
            decoration: BoxDecoration(
                color: Colors.transparent,
                //n color: Color.fromARGB(255, 51, 51, 51),
